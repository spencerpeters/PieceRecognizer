...
hist = np.histogram(bnGrey, bins=np.arange(0, 256))

fig, axes = plt.subplots(1, 2, figsize=(8, 3))
axes[0].imshow(bnGrey, cmap=plt.cm.gray, interpolation='nearest')
axes[0].axis('off')
...
axes[0].imshow(bnGrey > 100, cmap=plt.cm.gray, interpolation='nearest')
axes[0].set_title('coins > 100')

axes[1].imshow(bnGrey > 150, cmap=plt.cm.gray, interpolation='nearest')
...
markers = np.zeros_like(bnGrey)
markers[bnGrey < 30] = 1
markers[bnGrey > 150] = 2
...
image_label_overlay = label2rgb(labeled_coins, image=bnGrey)
